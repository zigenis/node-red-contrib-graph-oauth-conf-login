...
module.exports = function (RED) {
    ...
    async function ExecuteLogin(flowContext, node, initiallogin) {
        ...
        try {
            var form = "";
            var clientid = node.encoding ? encodeURIComponent(node.clientid) : node.clientid;
            var clientsecret = node.encoding ? encodeURIComponent(node.clientsecret) : node.clientsecret;
            var code = node.encoding ? encodeURIComponent(node.code) : node.code;
            var tenantid = node.encoding ? encodeURIComponent(node.tenantid) : node.tenantid;
    ...
    function IsInitialDelegatedLogin(node, flowContext) {
        ...
        var flowclientid = flowContext.get(node.clientid);
        var flowclientsecret = flowContext.get(node.clientsecret);
        var flowtenantid = flowContext.get(node.tenantid);
    ...
    function SetInitialContext(flowContext, node) {
        ...
        flowContext.set(node.clientid, true);
        flowContext.set(node.clientsecret, true);
        flowContext.set(node.tenantid, true);
    ...
    RED.nodes.registerType("node-red-contrib-graph-oauth-conf-login", GraphOnBehalfLoginNode, {
        credentials: {
